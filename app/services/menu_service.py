...
class MenuService:
    ...
    async def read_menus(self):
        ...
        await self.cache_client.set('all', data)
        return data

    async def create_menu(self, menu_data):
        data = await self.menu_repository.create_menu(menu_data)
        await self.cache_client.set(f'{data.id}', data)
        await self.cache_client.clear_after_change(data.id)
        return data
    ...
    async def read_menu(self, menu_id: int | str):
        ...
        await self.cache_client.set(f'{menu_id}', data)
        return data
    ...
    async def del_menu(self, menu_id: int | str):
        ...
        await self.cache_client.delete(f'{menu_id}')
        await self.cache_client.clear_after_change(menu_id)
        return data
