...
class DishService:
    ...
    def read_dishes(self, submenu_id: int, menu_id: int):
        return self.dish_repository.read_dishes(submenu_id, menu_id)

    def create_dish(self, dish_data, submenu_id: int, menu_id: int):
        return self.dish_repository.create_dish(dish_data, submenu_id, menu_id)

    def read_dish(self, dish_id: int, submenu_id: int, menu_id: int):
        return self.dish_repository.read_dish(dish_id, submenu_id, menu_id)

    def update_dish(self, dish_id: int, dish_data, submenu_id: int, menu_id: int):
        return self.dish_repository.update_dish(dish_id, dish_data, submenu_id, menu_id)

    def del_dish(self, dish_id: int, submenu_id: int, menu_id: int):
