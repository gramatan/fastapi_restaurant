...
async def test_read_submenus(db_session: AsyncSession) -> None:
    ...
    async for session in db_session:
        ...
        submenu1 = await crud_submenu.create_submenu(session, submenu_in1, menu.id)
    async for session in db_session:
        submenu_in2 = SubMenuBase(title="Test SubMenu 2", description="This is test submenu 2")
        submenu2 = await crud_submenu.create_submenu(session, submenu_in2, menu.id)

        # Read submenus
        submenus = await crud_submenu.read_submenus(session, menu.id)
...
async def test_read_dishes(db_session: AsyncSession) -> None:
    ...
    async for session in db_session:
        ...
        submenu = await crud_submenu.create_submenu(session, submenu_in, menu.id)
    async for session in db_session:
        dish_in1 = DishBase(title="Test Dish 1", description="This is test dish 1", price=10.0)
        dish1 = await crud_dish.create_dish(session, dish_in1, menu.id, submenu.id)
    async for session in db_session:
        dish_in2 = DishBase(title="Test Dish 2", description="This is test dish 2", price=20.0)
        dish2 = await crud_dish.create_dish(session, dish_in2, menu.id, submenu.id)

        # Read dishes
        dishes = await crud_dish.read_dishes(session, submenu.id, menu.id)
