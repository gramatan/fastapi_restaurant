...
async def patch_menu(menu_id: int | str, menu: MenuBase, response: MenuService = Depends()) -> MenuResponse:
    return await response.update_menu(menu_id, menu)


@router.delete('/menus/{menu_id}')
async def delete_menu(menu_id: int | str, response: MenuService = Depends()) -> dict:
    return await response.del_menu(menu_id)


@router.get('/menus/ORM/{menu_id}')
async def get_menu_orm(menu_id: int | str, response: MenuService = Depends()) -> MenuResponse:
